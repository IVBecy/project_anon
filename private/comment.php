...
$src_query = "SELECT `comments` FROM `posts` WHERE `uname` = '$src_uname' AND `title` = '$like_title'";
...
$comments[$uname] = mysqli_real_escape_string($connection,e($msg));
$comments = json_encode($comments);
$comments = openssl_encrypt($comments,"AES-128-CBC",$src_id);
$append_q = "UPDATE `posts` SET `comments` = '$comments'  WHERE `uname` = '$src_uname' AND `title` = '$like_title'";
